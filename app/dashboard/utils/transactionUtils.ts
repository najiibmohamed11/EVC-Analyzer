...
export function getSummaryStats(transactions: transactionSchemaType): SummaryStats {
...
export function getContactStats(
  ...
  const averageCredit = creditTransactions.length > 0
    ? creditTransactions.reduce((sum, t) => sum + t.credit, 0) / creditTransactions.length
    : 0
  
  const averageDebit = debitTransactions.length > 0
    ? debitTransactions.reduce((sum, t) => sum + t.debit, 0) / debitTransactions.length
    : 0
...
export function getMonthlyTrends(transactions: transactionSchemaType) {
  ...
  return Array.from(monthlyMap.entries())
    .map(([key, data]) => ({
      month: new Date(key + '-01').toLocaleString('default', { month: 'short', year: 'numeric' }),
      income: data.income,
...
export function getTransactionInsights(transactions: transactionSchemaType) {
  ...
  const mostActiveDay = mostActiveDayIndex !== undefined ? dayNames[mostActiveDayIndex] : null

  // Average transaction size
  const allAmounts = transactions.map(t => t.credit + t.debit)
  const averageTransactionSize = allAmounts.reduce((sum, amt) => sum + amt, 0) / allAmounts.length
  ...
  const sortedByAmount = [...allTransactions].sort((a, b) => b.amount - a.amount)
  const largestTransaction = sortedByAmount[0]?.transaction || null
  const smallestTransaction = sortedByAmount[sortedByAmount.length - 1]?.transaction || null
